...
def test_performance():
    ...
    # 修改表头增加v3版本
    print(f"{'Size':<10} | {'Custom v1 (TFLOPS)':<18} | {'Custom v2 (TFLOPS)':<18} | {'Custom v3 (TFLOPS)':<18} | {'PyTorch (TFLOPS)':<18}")
    print("-" * 100)
    ...
    for n in sizes:
        ...
        #     sgemm.sgemm_fp32_v0(a, b)
        # torch.cuda.synchronize()
        # custom_v0_time = (time.time() - start) / num_runs
        ...
        # 修改输出格式包含v3结果
        print(f"{n:<10} | {custom_v1_tflops:<18.2f} | {custom_v2_tflops:<18.2f} | {custom_v3_tflops:<18.2f} | {torch_tflops:<18.2f}")
